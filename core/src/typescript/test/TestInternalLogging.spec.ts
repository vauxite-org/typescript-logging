import {$INTERNAL_LOGGING_SETTINGS$, Log4TSProvider} from "../main/typescript-logging";
...
describe("Test internal logging", () => {
  ...
  test("Should log creating provider", () => {
    const messages: string[] = [];
    $INTERNAL_LOGGING_SETTINGS$.setInternalLogLevel(InternalLogLevel.Debug);
    $INTERNAL_LOGGING_SETTINGS$.setOutput(msg => messages.push(msg));
    ...
    $INTERNAL_LOGGING_SETTINGS$.setInternalLogLevel(InternalLogLevel.Error);
